      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h3 className="text-sm sm:text-base text-[color:var(--color-fg-muted)] mt-2">
            Mevcut sisteminizle{" "}
            <span className="text-[color:var(--color-fg)] font-medium">uyumlu çalışır</span>
          </h3>
        </motion.div>
        ...
        <div className="flex flex-wrap justify-center items-center gap-2.5 sm:gap-3">
          {integrations.map((name, i) => (
            <motion.div
              ...
              className="px-3.5 sm:px-4 py-2 rounded-full bg-[color:var(--color-elevated)] border border-[color:var(--color-border)] text-xs sm:text-sm text-[color:var(--color-fg-secondary)] hover:border-brand-soft/40 hover:text-[color:var(--color-fg)] hover:bg-brand/5 transition-colors"
            >
              {name}
            </motion.div>
          ))}
