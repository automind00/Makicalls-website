...
import type { ChannelKey, SectorConfig } from "@/lib/sectors";
...
> = {
  voice: {
    ...
    title: "Sesli Arama",
    body: "Gelen her çağrıyı Türkçe karşılar, soruları yanıtlar, randevu açar.",
    badge: "Telefon entegre",
  },
  whatsapp: {
  ...
  web: {
    ...
    title: "Web Chat",
    body: "Sitenize gömülü sohbet widget'ı. Ziyaretçi gezerken anında destek alır.",
    badge: "Tek satır kod",
  },
  instagram: {
    icon: <InstagramIcon className="w-7 h-7" />,
    title: "Instagram DM",
    body: "Reklam ve postlardan gelen DM'leri otomatik yanıtlar, randevuya yönlendirir.",
    badge: "Reklam ROI artar",
  },
};

export default function SectorOmnichannel({ sector }: { sector: SectorConfig }) {
  const keys = sector.channels ?? (["voice", "whatsapp", "web", "instagram"] as ChannelKey[]);

  const channels = keys.map((k) => {
    const override = sector.channelCopy?.[k];
    const base = DEFAULT_CHANNELS[k];
    return {
      icon: base.icon,
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="mt-4 text-sm sm:text-base text-[color:var(--color-fg-muted)]">
            Aynı marka tonu, aynı kurallar, aynı müşteri geçmişi — kanal ne olursa olsun.
          </p>
        </motion.div>
          ...
          {channels.map((c, i) => (
            ...
            >
              <div className="h-full bg-[color:var(--color-elevated)] rounded-2xl p-5 lg:p-6 shadow-[var(--shadow-md)]">
                ...
                <h3 className="text-base font-semibold text-[color:var(--color-fg)] mb-2">
                  {c.title}
                </h3>
