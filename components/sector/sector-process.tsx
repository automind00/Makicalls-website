...
import type { SectorConfig, SectorProcessStep } from "@/lib/sectors";

const DEFAULT_STEPS: SectorProcessStep[] = [
  {
    n: "01",
    title: "İşletme bilgilerinizi paylaşın",
    body: "Tek kısa form: çalışma saatleri, hizmet listesi, kurallar, SSS, marka tonu.",
    time: "Sizden 15 dk",
  },
  {
    n: "02",
    title: "AI'yı size göre eğitiyoruz",
    body: "Bilgilerinizi alıyoruz, ses karakterini ayarlıyoruz, sistemlerinizle entegre ediyoruz.",
    time: "Bizim işimiz",
  },
  {
    n: "03",
    title: "Yayına geçer, raporları paylaşırız",
    body: "Asistan canlıda. Her hafta size çağrı, dönüşüm ve müşteri soru raporu gelir.",
    time: "Sürekli",
  },
];

export default function SectorProcess({ sector }: { sector: SectorConfig }) {
  const steps = sector.processSteps ?? DEFAULT_STEPS;
  return (
          ...
          <div className="grid md:grid-cols-3 gap-8 md:gap-6 relative">
            {steps.map((s, i) => (
              ...
              >
                ...
                <h3 className="text-lg font-semibold text-[color:var(--color-fg)] mb-2">
                  {s.title}
                </h3>
                <p className="text-sm text-[color:var(--color-fg-muted)] leading-relaxed mb-3">
                  {s.body}
                </p>
